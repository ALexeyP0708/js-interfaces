...
class InterfaceManager {
	...
	constructInterface(props=false){
		...
		if(self instanceof InterfaceManager){
		...
		for(let property in self.__interfaces__){
			if(!(property in self)){
				InterfaceManager.prototype.renderMesseges(['Missing property "'+self.constructor.name+'::'+property+'".']);
				continue;
			}
			let criteries_methods=[];
			for(let iface in self.__interfaces__[property]){					
				if(self.__interfaces__[property][iface].type=='property'){
					var check=InterfaceManager.prototype.checkingCriteries(self[property],self.__interfaces__[property][iface].criteries);
					if(check===false){
						InterfaceManager.prototype.renderMesseges(['Property "'+self.constructor.name+'::'+property+' does not correspond not to one of the criteries ',self.__interfaces__[property][iface]]);
						//break;
					}
				} else
				if(self.__interfaces__[property][iface].type=='method'){
					if(typeof self[property]!=='function'){
						InterfaceManager.prototype.renderMesseges(['Property "'+self.constructor.name+'::'+property+' is not method.']);
						continue;
					}
				...
				self[property]=function(...arg){
					for (let i in arg){  
						for(let j in criteries_methods){
							...
							if(i in criteria){
								var check=InterfaceManager.prototype.checkingCriteries(arg[i],criteria[i]);
							} else if('rest' in criteria){
								var check=InterfaceManager.prototype.checkingCriteries(arg[i],criteria.rest);
							} 
							if(check===false){
								InterfaceManager.prototype.renderMesseges(['Argument "'+i+'" in "'+self.constructor.name+'::'+property+'" method  does not correspond not to one of the criteries ',criteries_methods[j]]);
							}
						}
					}
					...
					for(let i in criteries_methods){
						...
						if('return' in criteria){
							var check=InterfaceManager.prototype.checkingCriteries(re,criteria.return);
						} 
						if(check===false){
							InterfaceManager.prototype.renderMesseges(['Return result in "'+self.constructor.name+'::'+property+'" method  does not correspond not to one of the criteries ',criteries_methods[i]]);
						}
					}
					return re;
				}
