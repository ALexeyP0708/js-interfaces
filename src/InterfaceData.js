...
export const interfaceData = Symbol('interfaceData')
export class InterfaceData {
  ...
  static set (ProtoClass, data = {}) {
    ...
    if (data.end_points !== undefined) {
      for (const point of data.end_points) {
    ...
    if (data.staticProps !== undefined) {
      for (const prop of Object.keys(data.staticProps)) {
        self.isBuilt = false
        self.staticProps[prop] = data.staticProps[prop]
      }
    }
    if (data.protoProps !== undefined) {
      for (const prop of Object.keys(data.protoProps)) {
        self.isBuilt = false
  ...
  static setEndPoints (ProtoClass, ...points) {
    const interfaceData = this.init(ProtoClass)
  ...
  static getEndPoints (ProtoClass) {
    if (typeof ProtoClass === 'object') {
      const proto = Object.getPrototypeOf(ProtoClass)
  ...
  static instanceOfInterface (object, Interface) {
    ...
    const interfaceData = this.get(ProtoClass)
    if (interfaceData === undefined) {
      return false
    }
    for (const iface of interfaceData.interfaces) {
